...
void showSpec(AVFormatContext *ctx) {
    ...
    // 每一个样本的一个声道占多少个字节
    qDebug() << "每一个样本的一个声道占多少个字节:" << av_get_bytes_per_sample((AVSampleFormat)params->format);
    // 编码 Id(可以看出采样格式)
    qDebug() << "编码 Id:" << params->codec_id;
    // AV_CODEC_ID_PCM_F32LE
    qDebug() << "AV_CODEC_ID_PCM_F32LE:" << AV_CODEC_ID_PCM_F32LE;
    // 每一个样本的一个声道占多少位(这个需要用到 avcodec 库)
    qDebug() << "每一个样本的一个声道占多少位:" << av_get_bits_per_sample(params->codec_id);
}
