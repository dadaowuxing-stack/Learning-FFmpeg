...
void FFmpegs::aacDecode(const char *inFilename, AudioDecodeSpec &out) {
    ...
    int ret = 0;
    // 用来存放堆区的输入文件数据(aac)
    // 加上 AV_INPUT_BUFFER_PADDING_SIZE 是为了防止某些优化过的 render 一次性读取过多导致越界
    ...
    if (!outFile.open(QFile::WriteOnly)) {
        qDebug() << "outFile open error:" << out.filename;
        goto end;
    }
